...
import {
  ...
  LayoutGrid, ChevronDown, Eye, Maximize2, Award, FileText, MonitorOff
} from 'lucide-react';
...
export default function TeacherSessionView() {
  ...
  const [selectedAnomalyStudent, setSelectedAnomalyStudent] = useState(null);
                ...
                <div className="relative aspect-video bg-black flex items-center justify-center overflow-hidden">
                  {imageUrl ? (
                    ...
                    <div className="flex flex-col items-center justify-center gap-2 opacity-50">
                      <MonitorOff size={32} className="text-slate-500" />
                      <span className="text-slate-500 text-xs font-bold uppercase tracking-wider">Screen Not Shared</span>
                    </div>
                  )}
        ...
        <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-md flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-5xl w-full h-[85vh] flex flex-col overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="flex-1 flex overflow-hidden">
              {/* Big Screen Stream */}
              <div className="flex-1 bg-black/90 rounded-2xl border border-slate-800 overflow-hidden flex items-center justify-center p-4 relative">
                {focusedStudent.imageUrl ? (
                  ...
                  <div className="flex flex-col items-center justify-center gap-3 opacity-50">
                    <MonitorOff size={48} className="text-slate-500" />
                    <span className="text-slate-500 text-sm font-bold uppercase tracking-widest">Screen Not Shared</span>
                  </div>
                )}
      ...
      {showAnomalies && (
        <div className="fixed right-4 top-16 bottom-24 w-80 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl flex flex-col z-30">
          <div className="flex items-center justify-between px-4 py-3 border-b border-slate-800">
            <div className="flex items-center gap-2">
              {selectedAnomalyStudent && (
                <button onClick={() => setSelectedAnomalyStudent(null)} className="text-slate-400 hover:text-white mr-1">
                  &lt; Back
                </button>
              )}
              <h3 className="text-white font-bold text-sm">
                {selectedAnomalyStudent ? `Logs — ${selectedAnomalyStudent.name}` : `Session Anomalies (${anomalies.length})`}
              </h3>
            </div>
            <button onClick={() => { setShowAnomalies(false); setSelectedAnomalyStudent(null); }} className="text-slate-400 hover:text-white">
              <X size={16} />
            </button>
          </div>
          <div className="flex-1 overflow-y-auto p-3 space-y-2">
            {anomalies.length === 0 ? (
              ...
              // Tier 1: Group by student
              Object.values(anomalies.reduce((acc, a) => {
                if (!acc[a.studentId]) {
                  acc[a.studentId] = { studentId: a.studentId, name: a.studentName, logs: [] };
                }
                acc[a.studentId].logs.push(a);
                return acc;
              }, {})).map(group => {
                const redCount = group.logs.filter(l => l.outputCode?.startsWith('r') || l.isRed).length;
                const hasRed = redCount > 0;
                return (
                  <div
                    key={group.studentId}
                    onClick={() => setSelectedAnomalyStudent({ id: group.studentId, name: group.name })}
                    className={`p-3 rounded-xl border cursor-pointer hover:border-violet-500/60 transition-all ${
                      hasRed ? 'bg-rose-500/10 border-rose-500/30' : 'bg-amber-500/10 border-amber-500/30'
                    }`}
                  >
                    <div className="flex items-center justify-between">
                      <p className="text-white text-xs font-bold">{group.name}</p>
                      <span className={`px-2 py-0.5 text-[10px] font-mono font-bold rounded-md ${
                        hasRed ? 'bg-rose-500/20 text-rose-300' : 'bg-amber-500/20 text-amber-300'
                      }`}>
              ...
              // Tier 2: Specific student logs
              anomalies.filter(a => a.studentId === selectedAnomalyStudent.id).map(a => (
                <div
                  key={a.id}
                  onClick={() => focusOnStudent(a.studentId, a.studentName)}
                  className={`p-3 rounded-xl border cursor-pointer hover:border-violet-500/60 transition-all ${
                    (a.outputCode?.startsWith('r') || a.isRed)
                      ? 'bg-rose-500/10 border-rose-500/30' 
                      : 'bg-amber-500/10 border-amber-500/30'
                  }`}
                >
                  ...
                  {a.evidenceFrame && (
                    <button
                      ...
                      className="mt-2 text-[10px] text-violet-400 hover:text-violet-300 font-semibold flex items-center gap-1"
                    >
                      <Eye size={12} /> View 480p Screenshot
                    </button>
                  )}
